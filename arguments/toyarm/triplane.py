...
ModelHiddenParams = dict(
    ...
    defor_depth = 2,      # Number of FiLM layers
    net_width = 128,      # Hidden dimension
    grid_pe = 0,          # Positional encoding on grid features (0 = disabled)
    
    # ========== Control Signal Configuration ==========
    control_input_dim = 6,         # Input control dimension (e.g., 6-DOF)
    control_use_pe = True,         # Use positional encoding for control
    control_num_frequencies = 4,   # PE frequency bands
    control_hidden_dim = 64,       # Hidden dim for control MLP (if used)
    control_output_dim = None,     # None = use PE output directly (recommended)
                                   # Set to a value (e.g., 32) to project control features
    
    # ========== FiLM Fusion Configuration (NEW) ==========
    film_hidden_dim = 64,          # Hidden dim for FiLM γ/β generation
    film_use_residual = True,      # Use residual connection (recommended)
    
    # ========== Regularization ==========
    plane_tv_weight = 0.001,       # Total variation on spatial planes
    time_smoothness_weight = 0.0,  # Not used in TriPlane
    l1_time_planes = 0.0,          # Not used in TriPlane
    
    # ========== Deformation Flags ==========
    no_dx = False,    # Position deformation
    no_ds = True,     # Scale deformation (disabled by default)
